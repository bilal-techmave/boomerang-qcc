...
class TemplatesController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            $i = 0;
            $template = Template::findOrFail(decrypt($request->id));
            
            $template->update($request->except(['_token', 'question']));

            $existingFieldIds = TemplateField::where('template_id', $template->id)->pluck('id')->toArray();

            $receivedFieldIds = array_column($request->question, 'fielld_id');
            $idsToDelete = array_diff($existingFieldIds, $receivedFieldIds);

            TemplateField::whereIn('id', $idsToDelete)->delete();
    ...
    public function getFields(Request $request)
    {
        ...
        $html = view('templates.template-edit-html', compact('data'))->render();
        $count = count($data);
        if ($count > 0) {
            $data = TemplateField::where('template_id', $request->id)->first();
            $page_title = $data->t_page_name;
